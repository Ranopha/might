...
Because AgentMail had already received a `204` before this fix, the accepted component event did not redeliver automatically. The already Svix-verified, component-persisted event was reconciled once through the corrected internal mutation; no new webhook, message, or outbound was created. Might now has one application inbound event, `sendCount: 1`, matching real message/thread receipts, and `CONTACTED → REPLIED`. The existing Connections screen changed to `They replied` without refresh and retained that state after reload.

The owner then explicitly authorized the separate continuation decision. The existing `Yes, help me continue` control was used once, creating one continuation record and moving both the application connection and mail binding to `CONNECTED`; the already-open browser changed without refresh and a full reload retained the state. The outbound and inbound counts remained exactly one. This verifies the development `REPLIED → CONNECTED` consent transition without another email or any promise, agreement, price, payment, or schedule. An unassisted post-fix webhook run, production promotion, and repeatable full E2E acceptance remain open.
